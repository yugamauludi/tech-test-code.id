...
import { fetchContactDetail, contactPostEdit } from "../store/actions/actionCreator";
...
const EditContact = () => {
    ...
    useEffect(()=> {
        dispatch(fetchContactDetail(id))
    }, [])
    useEffect(() => {
        if (contactDetail.id) {
            setDataContact(contactDetail)
        }
    }, [contactDetail])
    const formSubmitEdit = (e) => {
        e.preventDefault()
        dispatch(contactPostEdit(dataContact))
        .then(data => {
            navigate('/')
            MySwal.fire({
                html: "Contact Successfully Edited",
                icon: 'success'
            })
        })
    }
    ...
    return (
        <>
            <div className="h-90 bg-white p-5 rounded-5" style={{ width: "50%", marginBottom: "5px", marginTop: "50px", marginLeft: "25%" }}>
                <h1 className="fw-bold mb-3">Edit Contact</h1>
                {/* <FormAddEdit /> */}
